...
qrcode.vidSuccess = function(stream) {
  ...
  if (qrcode.webkit) qrcode.video.src = window.webkitURL.createObjectURL(stream)
  else if (qrcode.moz) {
  ...
  if (qrcode.debug) {
    for (var y = 0; y < qRCodeMatrix.bits.Height; y++) {
      for (var x = 0; x < qRCodeMatrix.bits.Width; x++) {
        ...
        qrcode.imagedata.data[point] = qRCodeMatrix.bits.get_Renamed(x, y)
          ? 0
          : 0
        qrcode.imagedata.data[point + 1] = qRCodeMatrix.bits.get_Renamed(x, y)
          ? 0
          : 0
        qrcode.imagedata.data[point + 2] = qRCodeMatrix.bits.get_Renamed(x, y)
          ? 255
          : 0
      }
    }
